...
class PSCWS4
{
    ...
    public function get_result()
    {
        ...
        while (($off = ($off + $clen)) < $len) {
            ...
            } elseif (($pflag & PSCWS4_PFLAG_WITH_MB) && 1 === $clen) {
    ...
    public function get_tops($limit = 10, $xattr = '')
    {
        ...
        while ($tmpa = $this->get_result()) {
            foreach ($tmpa as $tmp) {
                ...
                // check attr filter
                if (count($attrs) > 0) {
                    if (true === $xmode && !isset($attrs[$tmp['attr']])) {
                        continue;
                    }
                    if (false === $xmode && isset($attrs[$tmp['attr']])) {
                        continue;
                    }
                }
    ...
    public function _rule_load($fpath)
    {
        ...
        while ($buf = fgets($fd, 512)) {
            ...
            if (':' === $ch) {
                ...
                $pval = trim($pval);
                if ('line' === $pkey) {
                    $rbl = ('n' === strtolower(substr($pval, 0, 1)) ? false : true);
                } elseif ('tf' === $pkey) {
                    $item['tf'] = (float) $pval;
                } elseif ('idf' === $pkey) {
                    $item['idf'] = (float) $pval;
                ...
                elseif ('znum' === $pkey) {
                    if ($pos = strpos($pval, ',')) {
                        $item['zmax'] = (int) (trim(substr($pval, $pos + 1)));
                ...
                } elseif ('include' === $pkey || 'exclude' === $pkey) {
                    $clude = 0;
    ...
    {
        return ($f & (PSCWS4_ZFLAG_SYMBOL | PSCWS4_ZFLAG_ENGLISH)) || (PSCWS4_ZFLAG_WHEAD === ($f & (PSCWS4_ZFLAG_WHEAD | PSCWS4_ZFLAG_NR2)));
    }
                ...
                while ($start < $end) {
                    ...
                    if ($pflag & PSCWS4_PFLAG_DIGIT) {
                        if (!$this->_is_digit($cx)) {
                            if (($pflag & PSCWS4_PFLAG_ADDSYM) || 0x2e !== $cx || !$this->_is_digit(ord($txt[$start + 1]))) {
                                break;
                            }
                        ...
                        if (!$this->_is_alpha($cx)) {
                            ...
                            if (($pflag & PSCWS4_PFLAG_ADDSYM) || 0x27 !== $cx || !$this->_is_alpha(ord($txt[$start + 1]))) {
                                break;
                            }
                            $pflag |= PSCWS4_PFLAG_ADDSYM;
                        }
    ...
    public function _mset_word($i, $j)
    {
        ...
        if ((false === $item) || (($this->_mode & PSCWS4_IGN_SYMBOL)
        && !($item['flag'] & PSCWS4_ZFLAG_ENGLISH) && 'un' === $item['attr'])) {
            return;
        }
        ...
        if ($this->_mode & PSCWS4_DUALITY) {
            $k = $this->_zis;
            if ($i === $j && !($item['flag'] & PSCWS4_ZFLAG_ENGLISH) && 'un' === $item['attr']) {
                $this->_zis = $i;
                if ($k < 0) {
                    return;
                }
        ...
        if (($j > $i) && ($this->_mode & (PSCWS4_MULTI_ZMAIN | PSCWS4_MULTI_ZALL))) {
            ...
            do {
                ...
                if (!($this->_mode & PSCWS4_MULTI_ZALL) && !strstr('jnv', substr($wmap[$i][$i]['attr'], 0, 1))) {
                    continue;
                }
                $this->_put_res($zmap[$i]['start'], $wmap[$i][$i]['idf'], $zmap[$i]['end'] - $zmap[$i]['start'], $wmap[$i][$i]['attr']);
            } while (++$i <= $j);
